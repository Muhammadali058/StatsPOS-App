package com.example.statspos.presentation.viewmodels.accounts.account_categories
...
@HiltViewModel
class AccountCategoriesViewModel @Inject constructor(
    private val api: AccountCategoriesRepository
) : ViewModel() {

    // region ScreenState
    data class ScreenState(
        val list: List<AccountCategories> = emptyList(),
        val totalAccountCategories: Int = 0,
...
//                return@launch
            ...
            when (val result = api.loadAccountCategories(params)) {
                ...
                is Resource.Success -> {
                    ...
                    val resultList =
                        Gson().getListOf<AccountCategories>(result.data.get("rows").asJsonArray)
                    state.update {
                        it.copy(
                            list = resultList,
                            totalAccountCategories = resultTotal,
                        )
                    }
                }
            }
