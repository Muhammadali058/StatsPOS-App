...
class AddUpdateSubBarcodeViewModel @Inject constructor(
    private val subBarcodesRepo: SubBarcodesRepository,
) : ViewModel() {
    // region ScreenState
...
//        onEvent(UiEvent.ShowMessage(message ?: ""))
    ...
    fun insertOrUpdateData(onSuccess: () -> Unit) {
        viewModelScope.launch {
            ...
            val result = if (state.value.isUpdate) {
                ...
                subBarcodesRepo.updateSubBarcode(subBarcode)
            } else {
                subBarcodesRepo.insertSubBarcode(subBarcode)
            }
    ...
    fun deleteData(id: Long, onSuccess: () -> Unit) {
        viewModelScope.launch {
            ...
            when (val result = subBarcodesRepo.deleteSubBarcode(id)) {
    ...
    fun editData(id: Long) {
        viewModelScope.launch {
            ...
            when (val result = subBarcodesRepo.getSubBarcode(id)) {
                is Resource.Error -> resultError(result.error)
                is Resource.Information -> resultInformation(result.message)
                is Resource.Success -> {
                    resultSuccess()
