...
class PackagesViewModel @Inject constructor(
    ...
    data class ScreenState(
        val list: List<Packages> = emptyList(),
...
//                return@launch
            ...
            when (val result = api.loadPackages(params)) {
                ...
                is Resource.Success -> {
                    ...
                    val resultTotal =
                        result.data.get("total").asJsonObject.get("totalPackages").asInt
                    val resultList =
                        Gson().getListOf<Packages>(result.data.get("rows").asJsonArray)
                    state.update {
                        it.copy(
                            list = resultList,
                            totalPackages = resultTotal,
                        )
                    }
                }
            }
