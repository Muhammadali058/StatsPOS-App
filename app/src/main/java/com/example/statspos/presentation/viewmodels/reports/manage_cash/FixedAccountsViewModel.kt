...
@HiltViewModel
class FixedAccountsViewModel @Inject constructor(
...
//                return@launch
            ...
            when (val result = api.loadFixedAccounts(params)) {
                ...
                is Resource.Success -> {
                    ...
                    val resultTotal =
                        result.data.get("total").asJsonObject.get("totalFixedAccounts").asInt
                    val resultList =
                        Gson().getListOf<Accounts>(result.data.get("rows").asJsonArray)
                    state.update {
                        it.copy(
                            list = resultList,
                            totalAccounts = resultTotal,
                        )
                    }
                }
            }
                ...
                is Resource.Success -> {
                    ...
                    state.update {
                        it.copy(
                            ...
                            totalAccounts = state.value.totalAccounts - 1,
                        )
                    }
