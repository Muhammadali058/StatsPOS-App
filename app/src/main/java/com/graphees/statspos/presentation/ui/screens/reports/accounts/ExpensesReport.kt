...
fun expensesReport(
    ...
    // region Document
    val file = File(context.cacheDir, "Expenses_Report_${System.currentTimeMillis()}.pdf")
    if (file.exists()) {
        file.delete()
    }
    ...
    // region Header
    // ---------------- Report Title ----------------
    val title = Paragraph("${totalReport.accountName} Expenses")
        .setBold()
        .setFontSize(24f)
...
//    document.add(Paragraph("\n"))
    ...
    // region Details
    // ---------------- Table ----------------
    val columnWidths = floatArrayOf(1f, 3f, 1f)
    ...
    val paragraph = Paragraph()
        ...
        .add(
            Div()
                ...
                .add(
                    Paragraph(totalReport.totalEntries.toString())
                        .setTextAlignment(TextAlignment.CENTER)
                )
        )
