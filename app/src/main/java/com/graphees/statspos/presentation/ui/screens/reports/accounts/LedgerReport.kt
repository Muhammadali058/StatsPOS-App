...
fun ledgerReport(
    ...
    // region Document
    val file = File(context.cacheDir, "Ledger_Report_${System.currentTimeMillis()}.pdf")
    if (file.exists()) {
        file.delete()
    }
...
//    document.add(Paragraph("\n"))
    ...
    val paragraph = Paragraph()
        .add(Text("Debit: ").setFont(boldFont))
        .add(
            Div()
    ...
    paragraph
        .add(Text("Credit: ").setFont(boldFont))
        .add(
            Div()
                .setWidth(UnitValue.createPointValue(100f))
                .setBorderBottom(SolidBorder(ColorConstants.BLACK, 1f))
    ...
    val totalCell = Cell()
        .add(
            Paragraph()
                .add(Text("Balance: ").setFont(boldFont))
                .add(
                    Div()
                        .setWidth(UnitValue.createPointValue(120f))
                        .setBorderBottom(SolidBorder(ColorConstants.BLACK, 1f))
                        .add(
                            Paragraph("${HP.formatDecimal(abs(totalReport.newBalance!!))} ${if(totalReport.newBalance!! > 0) "R" else "P"}")
                                .setTextAlignment(TextAlignment.CENTER)
                        )
                )
        )
