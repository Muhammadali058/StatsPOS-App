...
fun stockItemsReport(
    context: Context,
    costHeading:String,
    itemsReport: List<StockItemsReport>,
    totalReport: TotalReport,
): File {
    // region Document
    val file = File(context.cacheDir, "Stock_Report_${System.currentTimeMillis()}.pdf")
    if (file.exists()) {
        file.delete()
    }
...
//        }
    ...
    itemsReport.forEach { item ->
        ...
        bodyTable.addCell(
            Cell().add(Paragraph(HP.formatDecimal(item.stockPcs)).setFontSize(REPORT_BODY_FONT_SIZE))
                .setTextAlignment(TextAlignment.CENTER)
        )

        if (HP.settings.saleCartons == true) {
            bodyTable.addCell(
                Cell().add(Paragraph(item.stockCrtn.toString()).setFontSize(REPORT_BODY_FONT_SIZE))
                    .setTextAlignment(TextAlignment.CENTER)
            )
        }
        ...
        bodyTable.addCell(
            Cell().add(Paragraph(item.warehouseName.toString()).setFontSize(REPORT_BODY_FONT_SIZE))
                .setTextAlignment(TextAlignment.CENTER)
        )
    ...
    val paragraph = Paragraph()
        ...
        .add(
            Div()
                ...
                .add(
                    Paragraph(HP.formatDecimal(totalReport.totalStockPcs))
                        .setTextAlignment(TextAlignment.CENTER)
                )
        )
        ...
        paragraph
            ...
            .add(
                Div()
                    ...
                    .setBorderBottom(SolidBorder(ColorConstants.BLACK, 1f))
                    .add(
                        Paragraph(totalReport.totalStockCrtn.toString())
                            .setTextAlignment(TextAlignment.CENTER)
                    )
            )
    }
