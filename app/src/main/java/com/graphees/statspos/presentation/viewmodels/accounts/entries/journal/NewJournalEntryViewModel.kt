package com.graphees.statspos.presentation.viewmodels.accounts.entries.journal
...
class NewJournalEntryViewModel @Inject constructor(
    ...
    data class ScreenState(
        ...
        val debitAccountId: Long = 0L,
        val creditAccountId: Long = 0L,
...
//        onEvent(UiEvent.ShowMessage(message ?: ""))
    ...
    fun onDebitAccountIdChange(value: Long) {
        state.update { it.copy(debitAccountId = value) }
    }

    fun onCreditAccountIdChange(value: Long) {
        state.update { it.copy(creditAccountId = value) }
    }

    fun onDebitAccountNameChange(value: String) {
        state.update { it.copy(debitAccountName = value) }
    }

    fun onCreditAccountNameChange(value: String) {
        state.update { it.copy(creditAccountName = value) }
    }
    ...
    private fun getFormData(): Entries {
        return Entries(
            debitAccountId = state.value.debitAccountId,
            creditAccountId = state.value.creditAccountId,
    ...
    private fun isValid(): Boolean {
        if (state.value.debitAccountId == 0L) {
            showMessage("Please select debit account")
            return false
        }

        if (state.value.creditAccountId == 0L) {
            showMessage("Please select credit account")
            return false
        }
