package com.graphees.statspos.presentation.viewmodels.accounts.entries.payment
...
@HiltViewModel
class NewPaymentEntryViewModel @Inject constructor(
...
//        onEvent(UiEvent.ShowMessage(message ?: ""))
    ...
    private fun changeNaration() {
        var naration = if (state.value.isCustomer) {
            if (state.value.customerId != 0L)
                "Cash Paid to " + state.value.customerName
            else
                ""
        } else {
            if (state.value.vendorId != 0L)
                "Cash Paid to " + state.value.vendorName
            else
                ""
        }
