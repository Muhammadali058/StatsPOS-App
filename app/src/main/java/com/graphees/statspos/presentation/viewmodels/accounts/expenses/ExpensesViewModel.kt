package com.graphees.statspos.presentation.viewmodels.accounts.expenses
...
class ExpensesViewModel @Inject constructor(
    ...
    data class ScreenState(
        val list: List<Expenses> = emptyList(),
...
//                return@launch
            ...
            when (val result = api.loadExpenses(params)) {
                ...
                is Resource.Success -> {
                    ...
                    val resultTotal =
                        result.data.get("total").asJsonObject.get("totalExpenses").asInt
                    val resultList =
                        Gson().getListOf<Expenses>(result.data.get("rows").asJsonArray)
                    state.update {
                        it.copy(
                            list = resultList,
                            totalExpenses = resultTotal,
                        )
                    }
                }
            }
                ...
                is Resource.Success -> {
                    ...
                    state.update {
                        it.copy(
                            ...
                            totalExpenses = state.value.totalExpenses - 1,
                        )
                    }
