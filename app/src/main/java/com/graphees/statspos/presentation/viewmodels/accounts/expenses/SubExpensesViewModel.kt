...
@HiltViewModel
class SubExpensesViewModel @Inject constructor(
...
//                return@launch
            ...
            when (val result = api.loadSubExpenses(params)) {
                ...
                is Resource.Success -> {
                    ...
                    val resultTotal =
                        result.data.get("total").asJsonObject.get("totalSubExpenses").asInt
                    val resultList =
                        Gson().getListOf<Accounts>(result.data.get("rows").asJsonArray)
                    state.update {
                        it.copy(
                            list = resultList,
                            totalSubExpenses = resultTotal,
                        )
                    }
                }
            }
