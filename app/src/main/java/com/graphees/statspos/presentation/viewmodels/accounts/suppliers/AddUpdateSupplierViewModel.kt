package com.graphees.statspos.presentation.viewmodels.accounts.suppliers
...
@HiltViewModel
class AddUpdateSupplierViewModel @Inject constructor(
...
//        onEvent(UiEvent.ShowMessage(message ?: ""))
    ...
    fun insertOrUpdateData(onSuccess: () -> Unit) {
        viewModelScope.launch {
            ...
            val supplier = getFormData()

            val result = if (state.value.isUpdate) {
                supplier.id = state.value.updateId
                api.updateSupplier(supplier)
            } else {
                api.insertSupplier(supplier)
            }
...
//                    clearTextboxes()
    ...
    fun editData(id: Long) {
        viewModelScope.launch {
            ...
            when (val result = api.getSupplier(id)) {
                is Resource.Error -> resultError(result.error)
                is Resource.Information -> resultInformation(result.message)
                is Resource.Success -> {
                    resultSuccess()

                    val supplier = Gson().get<Accounts>(result.data.asJsonObject)
