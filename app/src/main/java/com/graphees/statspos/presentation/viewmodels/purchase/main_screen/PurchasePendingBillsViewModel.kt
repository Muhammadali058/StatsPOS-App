package com.graphees.statspos.presentation.viewmodels.purchase.main_screen
...
@HiltViewModel
class PurchasePendingBillsViewModel @Inject constructor(
...
//                return@launch
            ...
            when (val result = api.loadPendingBills(params)) {
                ...
                is Resource.Success -> {
                    ...
                    val resultList =
                        Gson().getListOf<PurchaseBills>(result.data.get("rows").asJsonArray)
                    state.update {
                        it.copy(
                            list = resultList,
    ...
    fun deleteBill(id: Long, onSuccess: () -> Unit) {
        viewModelScope.launch {
            ...
            when (val result = api.deletePurchase(id, false)) {
                is Resource.Error -> resultError(result.error)
                is Resource.Information -> resultInformation(result.message)
                is Resource.Success -> {
                    resultSuccess()
