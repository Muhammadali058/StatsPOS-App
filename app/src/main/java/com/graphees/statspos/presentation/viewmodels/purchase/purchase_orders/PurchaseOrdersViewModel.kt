...
class PurchaseOrdersViewModel @Inject constructor(
    ...
    data class ScreenState(
        val list: List<PurchaseOrders> = emptyList(),
...
//                return@launch
            ...
            when (val result = api.loadPurchaseOrders(params)) {
                ...
                is Resource.Success -> {
                    ...
                    val resultTotal =
                        result.data.get("total").asJsonObject.get("totalPurchaseOrders").asInt
                    val resultList =
                        Gson().getListOf<PurchaseOrders>(result.data.get("rows").asJsonArray)
                    state.update {
                        it.copy(
                            list = resultList,
